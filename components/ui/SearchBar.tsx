...
export default function SearchBar({ activeTab }: Props) {
  ...
  return activeTab === 'card' ? (
    <View className={styles.logoSearchCont}>
      <View style={{  width: width *0.9 }} className={styles.searchContainer}>
    ...
    <View className="w-full flex justify-center pb-6 h-14">
      <Text className="text-white text-center font-bold text-3xl">Evently</Text>
    </View>
  );
}
...
const styles = {
  ...
  searchContainer: 'flex-row items-center h-12 rounded-full px-3 bg-[#191827] m-auto px-6',
  searchIcon: 'mr-4',
};
