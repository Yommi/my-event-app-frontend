...
export default function EventPage() {
  ...
  const openMaps = async () => {
    ...
    try {
      if (Platform.OS === 'ios') {
        const googleMapsSupported = await Linking.canOpenURL(
          googleMapsNativeURL,
        );
        ...
        const googleMapsSupported =
          await Linking.canOpenURL(googleMapsURL);
        if (googleMapsSupported) {
  ...
  return (
    <SafeAreaView className="bg-[#191827] flex-1">
      <ScrollView
        refreshControl={
          <RefreshControl
            refreshing={eventPageLoading}
            onRefresh={refreshEventPage}
          />
        }
            ...
            <View className={styles.otherEventInfoCont}>
              {selectedEvent.distance ? (
                <Text className={styles.infoValue}>
                  <Text className={styles.otherEventInfoType}>
                    Distance:{' '}
                  </Text>
              ...
              <Text className={styles.infoValue}>
                ...
                {selectedEvent.currency
                  ? selectedEvent.currency.toUpperCase()
                  : ''}{' '}
                {selectedEvent.price
                  ? selectedEvent.price.toFixed(2).toLocaleString()
                  : 'Free'}
              </Text>
...
const styles = {
  ...
  tagsContainer:
    'mt-6 mx-auto flex-row justify-center flex-wrap gap-x-4 gap-y-4',
  ...
  buttons:
    ' flex-row justify-center align-center p-3 mx-4 my-3 bg-purple-700 rounded-3xl',
